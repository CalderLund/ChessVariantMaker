...
class Knight(Piece):
    ...
    def valid_moves(self, board: Board) -> List[str]:
        valid = []
        original_position = Board.translate_pos(self.position)
        ...
        # moves in a list
        move_options = [pos1, pos2, pos3, pos4, pos5, pos6, pos7, pos8]

        # checking all possible moves
        for squares in move_options:
...
if __name__ == "__main__":
    ...
    print(Board(invalid_squares=board["C7"].valid_moves(board)))
    print(Board(invalid_squares=board["D5"].valid_moves(board)))
    print(Board(invalid_squares=board["F4"].valid_moves(board)))
